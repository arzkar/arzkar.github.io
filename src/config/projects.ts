/**
 * Open-source & Tooling
 *
 * Developer tools, CLIs, and automation utilities built to solve real workflow problems.
 */
export const projects = {
  intro:
    "Outside of client work, I build developer tools, CLIs, and automation utilities — often to scratch real workflow problems.",
  items: [
    {
      name: "Veritas",
      description:
        "A local-first, multi-agent AI system for adversarial due diligence on startup pitch decks. Dispatches specialized agents to extract zero-trust claims, find contradicting evidence, detect suspicious absence of proof, and propagate credibility scores through a Bayesian engine.",
      link: "https://github.com/arzkar/Veritas",
      skills: ["Python", "FastAPI", "LangGraph", "Next.js", "Ollama", "FAISS"],
    },
    {
      name: "Heimdall",
      description:
        "A real-time global intelligence dashboard aggregating and visualizing disparate data streams for geopolitical, financial, and technological insights.",
      link: "https://github.com/arzkar/heimdall",
      demo: "https://heimdall-puce.vercel.app/",
      skills: ["Next.js", "React", "TypeScript", "Tailwind CSS"],
    },
    {
      name: "F1 Race Delta",
      description:
        "A web application providing advanced Formula 1 race strategy analysis, delta timing simulations, and data visualizations utilizing a custom Monte Carlo engine.",
      link: "https://github.com/arzkar/F1RaceDelta",
      demo: "https://f1-race-delta-blush.vercel.app/",
      skills: ["Python", "FastAPI", "React", "TypeScript"],
    },
    {
      name: "git-utils",
      description:
        "A CLI for performing various bulk operations on git repositories. Streamlines common git workflows for developers working with multiple repositories.",
      link: "https://github.com/arzkar/git-utils",
      skills: ["Python", "Go", "Node.js"],
    },
    {
      name: "adb-sync",
      description:
        "A CLI to sync between android & local system using adb. Simplifies file synchronization for Android developers and automation workflows.",
      link: "https://github.com/arzkar/adb-sync",
      skills: ["Python", "Go"],
    },
    {
      name: "fetch-ao3",
      description:
        "A library to fetch data from ArchiveOfOurOwn.org. Solves real-world problems in the fanfiction community with a clean, maintainable API.",
      link: "https://github.com/arzkar/fetch-ao3",
      skills: ["Python", "JavaScript"],
    },
  ],
};
